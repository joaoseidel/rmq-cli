        ...
        withConnection {
            ...
            if (unsafe) {
                if (messageOperations.reprocessMessage(message, it)) {
                    terminal.success("Reprocessed message #${message.id.value} from queue $fromQueue.")
                    return@withConnection
                }

                terminal.error("Failed to reprocess message from queue ${terminal.formatName(fromQueue)}. Check if the queue is empty or if there are connection issues.")
                return@withConnection
            }

            val operationResult = runBlocking {
                messageOperations.safeReprocessMessage(message, it)
            }

            if (operationResult.successful > 0) {
                terminal.success("Reprocessed #${message.id.value} from queue $fromQueue.")
            }

            if (operationResult.failed > 0) {
                terminal.error("Failed to reprocessed #${message.id.value} from queue $fromQueue.")
