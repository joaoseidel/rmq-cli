...
fun Message.toTable(terminal: Terminal, search: String = "") = table {
    ...
    val truncatedPayload = payload.truncateAroundPattern(search)
    val highlightedPayload = truncatedPayload.replace(searchPattern, terminal.theme.info(searchPattern))

    header { row("ID", "Exchange", "Routing key", "Payload", "Properties") }
    body { row(highlightedId, exchange, routingKey, highlightedPayload, properties) }
}.render(terminal)
...
fun List<Message>.toTable(terminal: Terminal, search: String = "") = table {
    ...
    body {
        forEach {
            ...
            val truncatedPayload = it.payload.truncateAroundPattern(search)
            val highlightedPayload = truncatedPayload.replace(searchPattern, terminal.theme.info(searchPattern))
            row(highlightedId, it.exchange, it.routingKey, highlightedPayload, it.properties)
        }
    }
}.render(terminal)