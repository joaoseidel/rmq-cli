...
const deliveryTagOf = (id: string) =>
  BigInt.asIntN(64, BigInt(`0x${id.slice(0, 16)}`));
...
describe("createMessageId", () => {
  ...
  it("distinguishes messages that differ only by payload", () => {
    const other = createMessageId({
      ...base,
      payload: Buffer.from('{"id":2}'),
    });
    expect(createMessageId(base)).not.toBe(other);
  });

  it("distinguishes messages that differ only by delivery tag", () => {
    expect(createMessageId(base)).not.toBe(
      createMessageId({ ...base, deliveryTagOrCount: 8 }),
    );
  });

  it("distinguishes messages that differ only by routing", () => {
    expect(createMessageId(base)).not.toBe(
      createMessageId({ ...base, routingKey: "order.updated" }),
    );
  });
